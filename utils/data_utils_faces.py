...
def get_data_faces(main_dir=None):
  ...
  dataset_folder = os.path.join(MAIN_DIR, 'data', 'cmu_faces')
  class_names = []
  class_id = 0
  #images are: (120, 128)
  for dir, subdirs, files in os.walk(dataset_folder):
    class_names = subdirs
    for class_name in class_names:
      file_names = [os.path.join(dataset_folder, class_name, x)
                    for x in os.listdir(os.path.join(dataset_folder, class_name))
                    if x.endswith('.pgm') and not x[x.rindex('.pgm')-1].isdigit()]
      dic_data[class_id] = [cv2.resize(cv2.imread(x, -1), (H, W)).reshape(H*W) for x in file_names]
      data_info[class_id] = file_names
      class_id += 1

  train_classes = range(15)
  val_classes = range(15, 20)
  train_data = {x: dic_data[x] for x in train_classes}
  val_data = {x: dic_data[x] for x in val_classes}

  intersection = set(train_data.keys()) & set(val_data.keys())
  assert not intersection, 'Train and validation data intersect.'
  logging.info('Number of labels in train dataset: %d.', len(train_data))
  logging.info('Number of labels in validation dataset: %d.', len(val_data))
  return train_data, val_data, data_info
...
def main():
  ...
  train_data, valid_data, data_info = get_data_faces()
