...
class SST2Dataset():
    ...
    def build_dataset(self, matrix_format=True):
        ...
        MAPS_PATH = os.path.join(self.data_dir, 'STS', 'lm')
        TOKENS_PATH = os.path.join(self.data_dir, 'STS', 'tokens')
        ...
        if matrix_format:
            ...
            self.x_train, self.seqlen_train, self.y_train   = self.convert_to_matrix(idl.c_inputs[idxTr], idl.labels[idxTr])
            self.x_test, self.seqlen_test, self.y_test      = self.convert_to_matrix(idl_val.c_inputs[idxTe_val], idl_val.labels[idxTe_val])
            self.x_val, self.seqlen_val, self.y_val         = self.convert_to_matrix(idl_val.c_inputs[idxTr_val], idl_val.labels[idxTr_val])
        else:
            # input data: [batch_size, seq_max_len, vocab_size] [256, 32, 65]
            self.x_train, self.seqlen_train, self.y_train   = idl.c_inputs[idxTr], idl.seq_lens[idxTr], idl.labels[idxTr]
            self.x_test, self.seqlen_test, self.y_test      = idl_val.c_inputs[idxTe_val], idl_val.seq_lens[idxTe_val], idl_val.labels[idxTe_val]
            self.x_val, self.seqlen_val, self.y_val         = idl_val.c_inputs[idxTr_val], idl_val.seq_lens[idxTr_val], idl_val.labels[idxTr_val]
    ...
    def matrix_to_text(self, x_batch, char_mode=True):
        dictionary = self.idl.DL.idToc if char_mode else self.idl.DL.idTow
        output = []
